...
const int priority_bonus[18][3] = {{1920, 1080, 8}, {1680, 1050, 2}, {1600, 900, 4}, {1280, 1024, 5}, {1440, 900, 5}, 
	{1366, 768, 24}, {1360, 768, 2}, {1280, 800, 7}, {1280, 720, 3}, {1024, 768, 13}, {1024, 600, 1}, {800, 600, 1}, 
	{480, 800, 3}, {360, 640, 4}, {320, 568, 4}, {320, 534, 1}, {320, 480, 3}, {240, 320, 1}};
...
// TODO genera crash
void resize(char *path, int q, int wid, int hei, char *cache) {
...
char *access_cache(char *path, int h, int w){
	...
	sprintf(query, "SELECT cache_path FROM cache WHERE orig_path = '%s' AND height = '%d' AND width = '%d'", path, h, w);
	...
	if (row == NULL){
		mysql_close(mysql); // close connection
		return path; // not found in database
	}
	...
	sprintf(query, "UPDATE cache SET priority = priority + 1 WHERE cache_path = '%s'", new_path);
	...
	// performs query
	if (mysql_query(mysql, query) != 0){  // fallisce
		perror("Error in mysql_query");
		mysql_close(mysql); // close connection
		return NULL;
	}
...
int insert_db(char *path, int h, int w, char *new_path, int priority){
		char query[MAX_BUF];
		MYSQL *mysql;

		mysql = db_connect();
		if (mysql == NULL){
			perror("Error in db connection");
			return 0;
		}
				
		sprintf(query, "INSERT INTO cache (orig_path, cache_path, height, width, priority) VALUES ('%s', '%s', '%d', '%d', '%d')", path, new_path, h, w, priority);
...
int db_cache_count(void){
	...
	sprintf(query, "SELECT COUNT(*) FROM cache");
...
int delete_cache(void){
	...
	sprintf(query, "SELECT cache_path FROM cache WHERE priority = (SELECT MIN(priority) FROM cache)");
	...
	memset(query, 0, sizeof(query));
	sprintf(query, "DELETE FROM cache ORDER BY priority LIMIT 1");
...
char *insert_cache(char *path, int height, int width){
	char new_path[MAX_BUF];
	char *image_name;
	int bonus = 0; // TODO assegnazione dei bonus
	char dim[MAX_BUF];
	int count;
	
	sprintf(dim, "%dx%d", height, width);
	...
	resize(path, q, width, height, new_path); // resize image and insert it into cache
	
	count = db_cache_count();
	printf("COUNT: %d\n", count);
	if (count == MAX_CACHE){
		if (delete_cache() == 0){
			return NULL;
	...
	int i;
	for (i=0; i < 18; i++){
		if (height == priority_bonus[i][0] && width == priority_bonus[i][1]){
			bonus = priority_bonus[i][2];
			break;
		}
	}
	if (insert_db(path, height, width, new_path, MAX_PRIORITY + bonus) == 0){
		return NULL;
	}
